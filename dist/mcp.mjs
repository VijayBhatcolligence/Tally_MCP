...
export async function registerMcpServer() {
    ...
    const importResult = (resp) => ({
        isError: !resp.ok,
        content: [{ type: 'text', text: JSON.stringify(resp) }]
    });
    const runImport = async (fn, ...fnArgs) => {
        try {
            return importResult(await fn(...fnArgs));
        }
        catch (err) {
            return importResult({
                ok: false,
                ...
                lastVoucherId: 0,
                lastMasterId: 0,
                lineError: err?.message || String(err),
                raw: ''
            });
        }
    };
    const targetCompanySchema = z.string().optional().describe('optional Tally company name. leave blank to use the currently open company');
    const ledgerEntrySchema = z.object({
        ledgerName: z.string().describe('exact Tally ledger name'),
        amount: z.number().describe('signed amount. Debit entries are negative in this MCP; credit entries are positive. Voucher entries must sum to zero'),
        billName: z.string().optional().describe('optional bill/reference allocation name'),
        billType: z.enum(['New Ref', 'Agst Ref', 'Advance', 'On Account']).optional().describe('optional bill allocation type')
    });
    mcpServer.registerTool('create-company', {
        title: 'Create Company',
        description: 'creates a Tally company master using Tally XML Import Data',
        inputSchema: {
            name: z.string(),
            financialYearFrom: z.string().describe('date in YYYY-MM-DD format'),
            booksBeginningFrom: z.string().optional().describe('date in YYYY-MM-DD format. defaults to financialYearFrom'),
    ...
    }, async (args) => runImport(createCompany, args));
    mcpServer.registerTool('create-group', {
        title: 'Create Group',
        description: 'creates a Tally account group master',
        inputSchema: {
            targetCompany: targetCompanySchema,
            name: z.string(),
            parent: z.string().optional().describe('parent group. defaults to Primary'),
            isSubLedger: z.boolean().optional(),
            isBillWise: z.boolean().optional(),
            useCostCenters: z.boolean().optional()
        },
        annotations: {
            readOnlyHint: false,
            openWorldHint: false
        }
    }, async (args) => runImport(createGroup, args));
    mcpServer.registerTool('create-ledger', {
        title: 'Create Ledger',
        description: 'creates a Tally ledger master',
        inputSchema: {
            targetCompany: targetCompanySchema,
            name: z.string(),
            parent: z.string().describe('parent group, for example Sundry Debtors, Sales Accounts, Purchase Accounts, Cash-in-Hand, Bank Accounts'),
            openingBalance: z.number().optional().describe('signed opening balance. Debit is negative and credit is positive'),
            isBillWise: z.boolean().optional(),
            useCostCenters: z.boolean().optional()
        },
        annotations: {
    ...
    }, async (args) => runImport(createLedger, args));
    mcpServer.registerTool('create-stock-group', {
        title: 'Create Stock Group',
        description: 'creates a Tally stock group master',
        inputSchema: {
            targetCompany: targetCompanySchema,
            name: z.string(),
    ...
    }, async (args) => runImport(createStockGroup, args));
    mcpServer.registerTool('create-stock-item', {
        title: 'Create Stock Item',
        description: 'creates a Tally stock item master',
        inputSchema: {
            targetCompany: targetCompanySchema,
            name: z.string(),
            parent: z.string().optional().describe('parent stock group. defaults to Primary'),
            baseUnit: z.string().describe('base unit symbol, for example Nos'),
            openingQuantity: z.number().optional(),
    ...
    }, async (args) => runImport(createStockItem, args));
    mcpServer.registerTool('create-unit', {
        title: 'Create Unit',
        description: 'creates a simple Tally unit of measure',
        inputSchema: {
            targetCompany: targetCompanySchema,
            symbol: z.string().describe('unit symbol, for example Nos, Kg, Pcs'),
    ...
    }, async (args) => runImport(createUnit, args));
    mcpServer.registerTool('create-godown', {
        title: 'Create Godown',
        description: 'creates a Tally godown/location master',
        inputSchema: {
            targetCompany: targetCompanySchema,
            name: z.string(),
            parent: z.string().optional().describe('parent godown. defaults to Primary')
        },
        annotations: {
            readOnlyHint: false,
            openWorldHint: false
        }
    }, async (args) => runImport(createGodown, args));
    mcpServer.registerTool('create-cost-center', {
        title: 'Create Cost Center',
        description: 'creates a Tally cost centre master',
        inputSchema: {
            targetCompany: targetCompanySchema,
            name: z.string(),
            parent: z.string().optional().describe('parent cost centre. defaults to Primary'),
            category: z.string().optional().describe('cost category. defaults to Primary Cost Category')
        },
        annotations: {
            readOnlyHint: false,
            openWorldHint: false
        }
    }, async (args) => runImport(createCostCenter, args));
    mcpServer.registerTool('create-currency', {
        title: 'Create Currency',
        description: 'creates a Tally currency master',
        inputSchema: {
            targetCompany: targetCompanySchema,
            symbol: z.string().describe('currency symbol/name, for example USD'),
    ...
    }, async (args) => runImport(createCurrency, args));
    const voucherInputSchema = {
        targetCompany: targetCompanySchema,
        date: z.string().describe('voucher date in YYYY-MM-DD format'),
        voucherNumber: z.string().optional(),
        partyLedgerName: z.string().optional().describe('optional party ledger. defaults to the first entry ledger'),
        narration: z.string().optional(),
        entries: z.array(ledgerEntrySchema).min(2).describe('balanced ledger entries. Debits are negative, credits are positive, and the total must be zero')
    };
    const voucherTools = [
        ['create-sales-voucher', 'Sales', 'Create Sales Voucher'],
        ['create-purchase-voucher', 'Purchase', 'Create Purchase Voucher'],
        ['create-payment-voucher', 'Payment', 'Create Payment Voucher'],
        ['create-receipt-voucher', 'Receipt', 'Create Receipt Voucher'],
        ['create-contra-voucher', 'Contra', 'Create Contra Voucher'],
        ['create-journal-voucher', 'Journal', 'Create Journal Voucher'],
        ['create-credit-note', 'Credit Note', 'Create Credit Note'],
        ['create-debit-note', 'Debit Note', 'Create Debit Note']
    ];
    for (const [toolName, voucherType, title] of voucherTools) {
        mcpServer.registerTool(toolName, {
            title,
            description: `creates a ${voucherType} accounting voucher in Tally. Entries use signed accounting convention: debit negative, credit positive`,
            inputSchema: voucherInputSchema,
            annotations: {
                readOnlyHint: false,
                openWorldHint: false
            }
        }, async (args) => runImport(createVoucher, args, voucherType));
    }
    mcpServer.registerTool('alter-ledger', {
        title: 'Alter Ledger',
        description: 'alters basic fields on an existing Tally ledger master',
        inputSchema: {
            targetCompany: targetCompanySchema,
            name: z.string(),
    ...
    }, async (args) => runImport(alterLedger, args));
    mcpServer.registerTool('alter-stock-item', {
        title: 'Alter Stock Item',
        description: 'alters basic fields on an existing Tally stock item master',
        inputSchema: {
            targetCompany: targetCompanySchema,
            name: z.string(),
            parent: z.string().optional(),
    ...
    }, async (args) => runImport(alterStockItem, args));
    mcpServer.registerTool('alter-voucher', {
        title: 'Alter Voucher',
        description: 'alters a voucher identified by voucher type, date, and voucher number',
        inputSchema: {
            voucherType: z.enum(['Sales', 'Purchase', 'Payment', 'Receipt', 'Contra', 'Journal', 'Credit Note', 'Debit Note']),
            ...voucherInputSchema
        },
        annotations: {
            readOnlyHint: false,
    ...
    }, async (args) => runImport(alterVoucher, args));
    mcpServer.registerTool('delete-voucher', {
        title: 'Delete Voucher',
        description: 'deletes a voucher identified by voucher type, date, and voucher number',
        inputSchema: {
            targetCompany: targetCompanySchema,
            voucherType: z.enum(['Sales', 'Purchase', 'Payment', 'Receipt', 'Contra', 'Journal', 'Credit Note', 'Debit Note']),
            date: z.string().describe('voucher date in YYYY-MM-DD format'),
    ...
    }, async (args) => runImport(deleteVoucher, args));
    return mcpServer;
}
